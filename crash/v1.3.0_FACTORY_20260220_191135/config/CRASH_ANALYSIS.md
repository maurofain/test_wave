...
- includere anche i file necessari al reflashing (`.bin` + `*_flash_args`) della stessa build.
...
Nel bundle sono presenti anche i file utili al reflashing:
- `build/test_wave.bin`, `build/bootloader.bin`, `build/partition-table.bin`
- `build/flash_app_args`, `build/flash_args`, `build/flash_project_args`, ecc.

Comandi tipici (dentro la cartella bundle):

```bash
# Reflash sola app
python -m esptool --chip esp32p4 write_flash @build/flash_app_args

# Reflash completo (bootloader + partition table + app + data)
python -m esptool --chip esp32p4 write_flash @build/flash_args
```
