...
"<div class='container'><div class='card'>"
        "<form id='f'><input type='file' id='i' accept='.bin' required><button type='submit'>{{013}}</button></form>"
        "<div style='margin-top:10px'><progress id='p' value='0' max='100' style='width:100%;height:20px;'></progress><div id='pct' style='margin-top:6px;font-weight:bold;'>{{019}}</div></div>"
        "<div id='s' style='margin-top:10px'></div></div></div><script src=\"/js/ota_03.js\"></script></body></html>"
;
...
const char *WEBPAGE_CONFIG_BODY =
        ...
        "<input type='text' id='location_name' name='location_name' placeholder='es: Carwashing' style='font-size:1.0em;'></div>"
        ...
        "<div class='sw-row'><label class='switch'><input type='checkbox' id='display_ads_en' name='display_ads_en'><span class='slider'></span></label><span>Attiva ADS</span></div>"
        "</div>"
        ...
        "</div>"
        "<div class='form-group'><label>Velocità Slideshow (ms)</label><input type='number' id='slideshow_speed_ms' name='slideshow_speed_ms' min='1000' max='30000' step='500' value='5000' style='width:140px' placeholder='Tempo transizione immagini'></div>"
        "</div>"
        ...
        "console.log('[CONFIG] scannerCfg initialized:', scannerCfg);"
        ...
        "if (scannerCfg) {"
        "    console.log('[CONFIG] scannerCfg found:', scannerCfg);"
        "    document.getElementById('scanner_en').checked = !!scannerCfg.en;"
        "    console.log('[CONFIG] scanner_en.checked set to:', !!scannerCfg.en);"
        ...
        "} else {"
        "    console.log('[CONFIG] scannerCfg is null, setting scanner_en.checked = false');"
        "    document.getElementById('scanner_en').checked = false;"
        ...
        "const slideshowSpeed=(timeoutsCfg&&timeoutsCfg.ad_rotation_ms)?parseInt(timeoutsCfg.ad_rotation_ms,10):5000;"
        "document.getElementById('slideshow_speed_ms').value=Math.max(1000,Math.min(30000,slideshowSpeed));"
        ...
        "document.getElementById('display_ads_en').checked=(typeof displayCfg.ads_en==='undefined')?true:!!displayCfg.ads_en;"
...
"scanner:{en:document.getElementById('scanner_en').checked,vid:document.getElementById('scanner_vid').value,pid:document.getElementById('scanner_pid').value,dpid:document.getElementById('scanner_dual_pid').value,cool:parseInt(document.getElementById('scanner_cooldown_ms').value)||10000},"
        "timeouts:{t_prg:(Math.max(1,Math.min(600,parseInt(document.getElementById('timeout_language_return_s').value)||60))*1000),t_lang:(Math.max(1,Math.min(600,parseInt(document.getElementById('timeout_language_exit_s').value)||60))*1000),ad_rotation_ms:Math.max(1000,Math.min(30000,parseInt(document.getElementById('slideshow_speed_ms').value)||5000))},"
        "display:{en:document.getElementById('display_en').checked,brt:parseInt(document.getElementById('lcd_bright').value),ads_en:document.getElementById('display_ads_en').checked},"
...
"<div class='container'>"
        "<div class='section'><h2>{{013}}</h2><div id='network'>{{026}}</div></div>"
        "<div class='section'><h2>{{015}}</h2><div id='partitions'>{{026}}</div></div>"
        "<div class='section'><h2>{{016}}</h2><div id='sd_card'>{{026}}</div></div>"
        "<div class='section'><h2>{{017}}</h2><div id='sensors'>{{026}}</div></div>"
        "<div class='section'><h2>{{018}}</h2><div id='env_info'>{{026}}</div></div>"
        "<div class='section'><h2>{{019}}</h2><div id='mdb_info'>{{026}}</div></div>"
        "</div>"
        "<script>"
...
"<div class='container'>"
        ...
        "window.addEventListener('DOMContentLoaded', async function(){ const stored = localStorage.getItem('httpservices_token'); try{ if(stored){ const respEl = document.getElementById('hs_response'); if(respEl) respEl.value = 'Token (localStorage): ' + stored; try{ showToken(stored); }catch(e){} } const r = await fetch('/api/config'); if(r.ok){ const c = await r.json(); if(c.server){ const serial = c.server.ser || c.server.serial || ''; const password = c.server.pwd || c.server.password || ''; if(serial) document.getElementById('hs_serial').value = serial; if(password) document.getElementById('hs_password').value = password; } } }catch(e){} });"
...
"<div class='card'><h2>{{033}}</h2>"
        "<div id='svc_grid' class='svc-grid'>{{034}}</div></div>"
        "<script>(function(){"
...
"<div class='container'>"
        ...
        "<span>{{016}}</span>"
        "</div>"
        "<button onclick='fmList()'>{{017}}</button>"
        "</div>"
        "<div class='row' style='margin-top:10px'>"
        "<input id='fm_file' type='file' />"
        "<button onclick='fmUpload()'>{{018}}</button>"
        "</div>"
        "<div class='row' style='margin-top:10px'>"
        "<button onclick='fmToggleAll()'>{{019}}</button>"
        "<div class='row-right'>"
        "<button id='btn_copy_to' onclick='fmCopyToOther()'>{{020}}</button>"
        "<button onclick='fmDownloadSelected()'>{{021}}</button>"
        "<button class='danger' onclick='fmDeleteSelected()'>{{022}}</button>"
        "</div>"
        "</div>"
        "<div id='fm_status' class='status'></div>"
        "<div id='fm_view_wrap' class='viewer' style='display:none'>"
        "<div class='viewer-head'><span id='fm_view_title'>{{023}}</span><button onclick='fmCloseView()'>{{024}}</button></div>"
        "<pre id='fm_view_content'></pre>"
        "</div>"
        "<table><thead><tr><th>{{025}}</th><th>{{026}}</th><th class='num'>{{027}}</th><th>{{028}}</th></tr></thead><tbody id='fm_list'></tbody></table>"
        "</div></div>"
        "<script>"
        ...
        "    fmSetStatus('{{035}} '+(j.files?j.files.length:0)+' {{036}} '+usedKb+' / '+totalKb+' {{037}}');"
        "  }catch(e){fmSetStatus('{{038}} '+e);}"
        "}"
        ...
        "  if(title) title.textContent='{{023}}';"
        ...
        "  if(!btn) return;"
        "  btn.textContent=fmStorage()==='sdcard'?'{{020}}':'{{057}}';"
        "}"
...
"<div class='container'>"
        ...
        "if(logs.length===0){container.innerHTML='{{020}}';return;}"
        "container.innerHTML='';"
        "logs.forEach(log=>{"
        ...
        "}catch(e){console.error(e);document.getElementById('logContainer').innerHTML='{{027}} '+e;}"
        "}"
        ...
        "  if(applyBtn){ applyBtn.addEventListener('click', async function(){ var container = document.getElementById('levelsContainer'); if(!container) return; var selects = container.querySelectorAll('select[data-tag]'); applyBtn.disabled=true; applyBtn.innerText='Applicazione...'; for(const s of selects){ var tag = s.getAttribute('data-tag'); var lvl = parseInt(s.value); try{ await setLogLevel(tag,lvl); }catch(e){console.error(e);} } applyBtn.disabled=false; applyBtn.innerText='{{016}}'; }); }\n"
...
"<div class='container'>"
        ...
        "  const r = await fetch('/api/config'); if(!r.ok){ console.warn('[TEST] Config fetch failed, showing all sections'); return; } const c = await r.json();"
        "  console.log('[TEST] Config loaded:', c);"
        "  /* sensors */"
        "  if(c.sensors){"
        "    console.log('[TEST] Sensors config:', c.sensors);"
        "    if(c.sensors.led_enabled === false){ const s=document.getElementById('section_led'); if(s) s.style.display='none'; }"
        "    if(c.sensors.io_expander_enabled === false){ const s=document.getElementById('section_ioexp'); if(s) s.style.display='none'; }"
        "    if(c.sensors.pwm1_enabled === false && c.sensors.pwm2_enabled === false){ const s=document.getElementById('section_pwm'); if(s) s.style.display='none'; }"
        "    if(c.sensors.rs232_enabled === false){ const s=document.getElementById('section_rs232'); if(s) s.style.display='none'; }"
        "    if(c.sensors.rs485_enabled === false){ const s=document.getElementById('section_rs485'); if(s) s.style.display='none'; }"
        "    if(c.sensors.rs485_enabled === false || (c.modbus && c.modbus.enabled === false)){ const s=document.getElementById('section_modbus'); if(s) s.style.display='none'; }"
        "    if(c.sensors.mdb_enabled === false){ const s=document.getElementById('section_mdb'); if(s) s.style.display='none'; }"
        "    if(c.sensors.eeprom_enabled === false){ const s=document.getElementById('section_eeprom'); if(s) s.style.display='none'; }"
        "    if(c.sensors.sd_card_enabled === false){ const s=document.getElementById('section_sd'); if(s) s.style.display='none'; }"
        "    if(c.sensors.temperature_enabled === false){ const s=document.getElementById('section_sht40'); if(s) s.style.display='none'; }"
        "  }"
        "  /* scanner */ if(c.scanner && c.scanner.enabled === false){ const s=document.getElementById('section_scanner'); if(s) s.style.display='none'; }"
        "  /* GPIO/SHT */ if(c.gpios && c.gpios.gpio33 && c.gpios.gpio33.mode !== 3){ const s=document.getElementById('section_gpio33'); if(s) s.style.display='none'; }"
        " }catch(e){console.error('[TEST] hideDisabledSections error:',e);} })();"
