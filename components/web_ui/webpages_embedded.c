...
const char *WEBPAGE_CONFIG_BODY =
        ...
        "document.getElementById('location_name').value=c.location_name || '';"
        "document.getElementById('num_programs').value=(c.num_programs&&[1,2,4,6,8,10].includes(Number(c.num_programs)))?c.num_programs:10;"
        "document.getElementById('latitude').value=(typeof c.latitude==='number')?c.latitude:0;"
        "document.getElementById('longitude').value=(typeof c.longitude==='number')?c.longitude:0;"
        ...
        "const ethCfg=(c.eth&&typeof c.eth==='object')?c.eth:{enabled:false,dhcp_enabled:true,ip:'',subnet:'',gateway:'',dns1:'',dns2:''};"
        "const wifiCfg=(c.wifi&&typeof c.wifi==='object')?c.wifi:{sta_enabled:false,dhcp_enabled:true,ssid:'',password:''};"
        "const ntpCfg=(c.ntp&&typeof c.ntp==='object')?c.ntp:{server1:'',server2:'',timezone_offset:0};"
        "const serverCfg=(c.server&&typeof c.server==='object')?c.server:{};"
        "const remoteLogCfg=(c.remote_log&&typeof c.remote_log==='object')?c.remote_log:{};"
        ...
        "const gpio33Cfg=(c.gpios&&c.gpios.gpio33&&typeof c.gpios.gpio33==='object')?c.gpios.gpio33:{mode:0,state:false};"
        "document.getElementById('eth_en').checked=!!ethCfg.enabled;"
        "document.getElementById('eth_dhcp').checked=!!ethCfg.dhcp_enabled;"
        "document.getElementById('eth_ip').value=(typeof ethCfg.ip==='string')?ethCfg.ip:'';"
        "document.getElementById('eth_subnet').value=(typeof ethCfg.subnet==='string')?ethCfg.subnet:'';"
        "document.getElementById('eth_gateway').value=(typeof ethCfg.gateway==='string')?ethCfg.gateway:'';"
        ...
        "document.getElementById('wifi_en').checked=!!wifiCfg.sta_enabled;"
        "document.getElementById('wifi_dhcp').checked=!!wifiCfg.dhcp_enabled;"
        "document.getElementById('wifi_ssid').value=(typeof wifiCfg.ssid==='string')?wifiCfg.ssid:'';"
        "document.getElementById('wifi_pwd').value=(typeof wifiCfg.password==='string')?wifiCfg.password:'';"
        "document.getElementById('ntp_en').checked=!!c.ntp_enabled;"
        "document.getElementById('ntp_server1').value=(typeof ntpCfg.server1==='string')?ntpCfg.server1:'';"
        "document.getElementById('ntp_server2').value=(typeof ntpCfg.server2==='string')?ntpCfg.server2:'';"
        "document.getElementById('ntp_timezone_offset').value=(typeof ntpCfg.timezone_offset==='number')?ntpCfg.timezone_offset:0;"
        "document.getElementById('server_en').checked=!!serverCfg.enabled;"
        ...
        "document.getElementById('server_serial').value=(typeof serverCfg.serial==='string'&&serverCfg.serial)?serverCfg.serial:'AD-34-DFG-333';"
        "document.getElementById('server_password').value=(typeof serverCfg.password==='string'&&serverCfg.password)?serverCfg.password:'c1ef6429c5e0f753ff24a114de6ee7d4';"
        "document.getElementById('remote_log_port').value=(typeof remoteLogCfg.server_port==='number')?remoteLogCfg.server_port:9514;"
        "document.getElementById('remote_log_broadcast').checked=!!remoteLogCfg.use_broadcast;"
        "document.getElementById('remote_log_sd').checked=!!remoteLogCfg.write_to_sd;"
        ...
        "document.getElementById('io_exp').checked=!!sensorsCfg.io_expander_enabled;"
        "document.getElementById('temp').checked=!!sensorsCfg.temperature_enabled;"
        "document.getElementById('led').checked=!!sensorsCfg.led_enabled;"
        "document.getElementById('led_count').value=(typeof sensorsCfg.led_count==='number'&&sensorsCfg.led_count>0)?sensorsCfg.led_count:16;"
        "document.getElementById('rs232').checked=!!sensorsCfg.rs232_enabled;"
        "document.getElementById('rs485').checked=!!sensorsCfg.rs485_enabled;"
        "document.getElementById('mdb').checked=!!sensorsCfg.mdb_enabled;"
        "const cctalkSensorEl=document.getElementById('cctalk');if(cctalkSensorEl){cctalkSensorEl.checked=(typeof sensorsCfg.cctalk_enabled==='undefined')?true:!!sensorsCfg.cctalk_enabled;}"
        "document.getElementById('eeprom').checked=(typeof sensorsCfg.eeprom_enabled==='undefined')?true:!!sensorsCfg.eeprom_enabled;"
        "document.getElementById('sd_card').checked=!!sensorsCfg.sd_card_enabled;"
        "document.getElementById('pwm1').checked=!!sensorsCfg.pwm1_enabled;"
        "document.getElementById('pwm2').checked=!!sensorsCfg.pwm2_enabled;"
        "if (scannerCfg) {"
        ...
        "    document.getElementById('scanner_dual_pid').value = scannerCfg.dual_pid ? ('0x' + scannerCfg.dual_pid.toString(16).padStart(4, '0').toUpperCase()) : '0x0000';"
        "    document.getElementById('scanner_cooldown_ms').value = scannerCfg.cooldown_ms ? scannerCfg.cooldown_ms : 10000;"
        "} else {"
        "    document.getElementById('scanner_cooldown_ms').value = 10000;"
        "}"
        "const languageTimeoutMs=(timeoutsCfg&&timeoutsCfg.language_return_ms)?parseInt(timeoutsCfg.language_return_ms,10):10000;"
        ...
        "if (typeof displayCfg.enabled === 'undefined') { try { const s = await (await fetch('/status')).json(); document.getElementById('display_en').checked = s.config && s.config.display ? !!s.config.display.enabled : true; } catch(e) { document.getElementById('display_en').checked = true; } } else { document.getElementById('display_en').checked = !!displayCfg.enabled; }" 
        "const brightVal=(typeof displayCfg.lcd_brightness==='number')?displayCfg.lcd_brightness:100;"
        ...
        "document.getElementById('g33_mode').value=(typeof gpio33Cfg.mode==='number')?gpio33Cfg.mode:0;"
        "document.getElementById('g33_state').checked=!!gpio33Cfg.state;"
        "if(remoteLogCfg.use_broadcast){const ipEl=document.getElementById('remote_log_ip'); if(ipEl){ipEl.disabled=true; ipEl.value='255.255.255.255';}}"
        "updateCurrentTime();"
        ...
        "num_programs:parseInt(document.getElementById('num_programs').value)||10,"
...
"<div class='card'><h2>🧩 Stato servizi</h2>"
        ...
        "const remoteLog=(c&&c.remote_log)?c.remote_log:{};"
        ...
        "rows.push(svcItemHtml('🕒','NTP',!!c.ntp_enabled,'ATTIVO','DISABILITATO'));"
