...
"<div class='container'>"
        ...
        "    <label style='display:inline-flex; align-items:center; gap:4px; padding:4px 8px; border-radius:4px; background:#2c3e50; color:#ecf0f1; font-size:12px;'><input type='checkbox' id='cctalk_comments'>Commenti</label>"
        "  </div>"
        "</div>"
        ...
        "      continue;"
        "    }"
        "    const color = isTx ? '#f1c40f' : (isRx ? '#ff4d4d' : '#bdc3c7');"
        "    if(currentType && currentType !== (isTx ? 'TX' : isRx ? 'RX' : null)){"
        "      flushGroup();"
        "    }"
        "    currentType = isTx ? 'TX' : isRx ? 'RX' : null;"
        "    if(highlightCoinEvents && highlightNextRx && isTx){ highlightNextRx = false; }"
        "    if(highlightCoinEvents && highlightNextRx && isRx){"
        "      currentGroup += '<span style=\"background:#f9e79f;color:#7b241c;font-weight:bold;padding:0 2px;border-radius:2px;\">.' + hex + '</span>';"
        "      continue;"
        "    }"
        "    if(forceHex || mode === 'HEX') currentGroup += '<span style=\"color:'+color+'\">.' + hex + '</span>';"
        "    else currentGroup += '<span style=\"color:'+color+'\">' + text + '</span>';"
        "  }"
        "  flushGroup();"
        "  return out;"
        "};"
        ...
        "if(res.cctalk) { const el=document.getElementById('cctalk_status'); const commentsEl=document.getElementById('cctalk_comments'); const showComments=!!(commentsEl && commentsEl.checked); if(el) { el.innerHTML = processLog(res.cctalk, 'cctalk_mode', {showActions:showComments, forceHex:true, highlightCoinEvents:true}); el.scrollTop = el.scrollHeight; } }"
