...
".card{background:white;padding:25px;margin:20px 0;border-radius:12px;box-shadow:0 4px 15px rgba(0,0,0,0.1);transition:.3s}"
        ...
        ".svc-on{color:#1e8449}.svc-off{color:#c0392b}"
;
...
"<div class='card'><h2>🧩 Stato servizi</h2>"
        ...
        "function svcItemHtml(icon,label,on,onTxt,offTxt){"
        "var txt=on?onTxt:offTxt;"
        "var cls=on?'svc-on':'svc-off';"
        "var dot=on?'🟢':'🔴';"
        ...
        "rows.push(svcItemHtml('📶','WiFi STA',hasIp(s.ip_sta),'ONLINE','OFFLINE'));"
        "rows.push(svcItemHtml('🔌','Ethernet',hasIp(s.ip_eth),'ONLINE','OFFLINE'));"
        ...
        "rows.push(svcItemHtml('🧩','Expansion board',expansionOn,expansionOnTxt,'DISABILITATA'));"
        "rows.push(svcItemHtml('💡','LED STRIP',!!sensors.led_enabled,'ATTIVO','DISABILITATO'));"
        "rows.push(svcItemHtml('🔤','RS232',!!sensors.rs232_enabled,'ABILITATA','DISABILITATA'));"
        "rows.push(svcItemHtml('🔠','RS485',!!sensors.rs485_enabled,'ABILITATA','DISABILITATA'));"
        "rows.push(svcItemHtml('🌡️','SHT40',!!sensors.temperature_enabled,'ATTIVO','DISABILITATO'));"
        "rows.push(svcItemHtml('〰️','PWM',pwmOn,pwmOnTxt,'DISABILITATO'));"
        "rows.push(svcItemHtml('📊','RemoteLogging',remoteOn,remoteOnTxt,'DISABILITATO'));"
        "rows.push(svcItemHtml('🎰','MDB',!!(s.mdb&&s.mdb.coin_online),'ONLINE','OFFLINE'));"
        "rows.push(svcItemHtml('💾','SD card',!!(s.sd&&s.sd.mounted),'MONTATA','NON MONTATA'));"
        "rows.push(svcItemHtml('🔁','CCtalk',!!sensors.cctalk_enabled,'ABILITATO','DISABILITATO'));"
...
"<div class='container'>"
        ...
        "  try{"
        "    const endpoint = (action === 'start') ? 'cctalk_start' : 'cctalk_stop';"
