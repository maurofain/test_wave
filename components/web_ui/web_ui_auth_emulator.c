...
esp_err_t emulator_page_handler_local(httpd_req_t *req)
{
    ...
    const char *extra_style =
        ...
        ".emu-layout{height:100%;display:flex;gap:10px}"
        ".emu-left{flex:0 0 27%;min-width:0;display:grid;grid-template-rows:repeat(8,1fr);gap:8px}"
        ".prog-btn{border:none;border-radius:8px;background:#2c3e50;color:#fff;font-size:17px;font-weight:bold;cursor:pointer;padding:6px 4px;line-height:1.1}"
        ".prog-btn.active{background:#e74c3c}"
        ".prog-btn:disabled{opacity:.45;cursor:not-allowed}"
        ".emu-main{flex:1;min-width:0;display:flex;flex-direction:column;gap:12px}"
        ...
        ".emu-gauge{flex:0 0 8%;min-width:44px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px}"
    ...
    const char *body =
        ...
        "  buttons.forEach(function(btn){const id=parseInt(btn.dataset.id||'0',10);const p=programMetaByButton[id];if(!p){btn.disabled=true;return;}const needed=(p.price_units||0);const enabled=!!p.enabled&&credit>0&&credit>=needed;btn.disabled=!enabled;});"
        "}"
