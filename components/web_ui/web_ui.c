...
/* uart_test_task moved to components/web_ui/web_ui_pages.c */
...
/* root_get_handler moved to components/web_ui/web_ui_pages.c */

/* logo_get_handler spostato in web_ui_common.c */

/* status_get_handler moved to components/web_ui/web_ui_pages.c */

/* OTA handlers moved to components/web_ui/web_ui_pages.c */

/* not_found_handler moved to components/web_ui/web_ui_pages.c (prototype in web_ui_internal.h) */
...
static esp_err_t test_page_handler(httpd_req_t *req)
{
    ...
    const char *body = 
        ...
        "<!-- HTTP Services quick-access (also available full in /httpservices) -->"
        "<div id='section_httpservices_quick' class='section collapsed'><h2>🔐 HTTP Services <span class='section-toggle-icon'>▸</span></h2>"
        "<div class='test-item'><span class='test-label'>Apri interfaccia completa</span><div class='test-controls'><a href='/httpservices' style='background:#3498db; color:#fff; padding:8px 12px; border-radius:4px; text-decoration:none;'>Apri /httpservices</a></div></div>"
        "<div class='test-item'><span class='test-label'>Token salvato</span><div class='test-controls'><input id='test_hs_token' type='text' readonly style='flex:1; padding:6px; font-family:monospace'> <button onclick='(function(){const t=document.getElementById('test_hs_token').value; if(!t) return; navigator.clipboard.writeText(t).then(()=>{const s=document.getElementById('test_hs_status'); if(s){ s.style.color='#27ae60'; s.innerText = 'Token copiato'; setTimeout(()=>{s.innerText=''; s.style.color='';},1500);} }).catch(()=>{});})()' style='margin-left:8px; padding:6px 8px;'>📋 Copia</button> <button onclick='(function(){ localStorage.removeItem('httpservices_token'); document.getElementById('test_hs_token').value=''; const s=document.getElementById('test_hs_status'); if(s){ s.style.color='#c0392b'; s.innerText = 'Token rimosso'; setTimeout(()=>{s.innerText=''; s.style.color='';},1500);} })()' style='margin-left:8px; padding:6px 8px; background:#c0392b; color:#fff;'>Rimuovi</button></div></div>"
        "<div class='test-item'><div class='test-controls'><button onclick='(async function(){ const s=document.getElementById('test_hs_status'); s.innerText=''; const t=document.getElementById('test_hs_token').value; if(!t){ s.style.color='#c0392b'; s.innerText='Nessun token disponibile'; return; } try{ const r = await fetch('/api/config',{method:'GET', headers:{'Authorization':'Bearer '+t}}); if(r.ok){ s.style.color='#27ae60'; s.innerText='Chiamata OK — response JSON presente'; } else { s.style.color='#c0392b'; s.innerText='HTTP '+r.status; } }catch(e){ s.style.color='#c0392b'; s.innerText='Errore rete'; console.error(e);}  })()' style='background:#2ecc71; color:#fff; padding:6px 8px;'>Test token (GET /api/config)</button></div></div>"
        "<div id='test_hs_status' class='status-box'></div></div>"
        "<script> (function(){ try{ const stored = localStorage.getItem('httpservices_token'); const el = document.getElementById('test_hs_token'); if(el && stored) el.value = stored; }catch(e){} })(); </script>"
        ...
        "  if(!btn){ console.error('toggleSimpleTest: button not found', btnId); return; }"
        ...
        "  btn.disabled = true;"
        "  try {"
        "    console.debug('toggleSimpleTest ->', prefix, btnId, label, 'isRunning=', isRunning);"
        "    if(isRunning){"
        "      btn.innerText = '▶️ Inizia ' + label; btn.style.background = '';"
        "      const res = await runTest(prefix + '_stop');"
        "      if(res && res.error && statusBox) statusBox.innerHTML += '<div class=\"result\">❌ ' + res.error + '</div>';"
        "    } else {"
        "      btn.innerText = '⏹️ FERMA ' + label; btn.style.background = '#e74c3c';"
        "      const res = await runTest(prefix + '_start');"
        "      if(res && res.error){"
        "        // revert UI if server returned error"
        "        btn.innerText = '▶️ Inizia ' + label; btn.style.background = '';"
        "        if(statusBox) statusBox.innerHTML += '<div class=\"result\">❌ ' + res.error + '</div>';"
        "      }"
        "    }"
        "  } catch(e){"
        "    console.error('toggleSimpleTest exception', e);"
        "    if(statusBox) statusBox.innerHTML += '<div class=\"result\">❌ ' + (e.message||e) + '</div>';"
        "  } finally {"
        "    btn.disabled = false;"
        ...
        "  console.debug('runTest ->', test, params);"
        "try{"
        ...
        "console.debug('runTest response', r.status, result);"
        "const time = new Date().toLocaleTimeString();"
...
static esp_err_t httpservices_page_handler(httpd_req_t *req)
{
