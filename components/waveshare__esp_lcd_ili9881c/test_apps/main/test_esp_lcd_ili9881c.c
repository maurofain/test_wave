...
static void test_init_lcd(void)
{
    ...
    TEST_ESP_OK(esp_lcd_new_panel_ili9881c(mipi_dbi_io, &panel_config, &panel_handle));
...
TEST_CASE("test ili9881c to draw pattern with MIPI interface", "[ili9881c][draw_pattern]")
{
...
TEST_CASE("test ili9881c to draw color bar with MIPI interface", "[ili9881c][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test ili9881c to mirror with MIPI interface", "[ili9881c][mirror]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
