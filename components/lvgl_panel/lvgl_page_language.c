...
typedef struct {
    const char *code;
    const char *label;
    const lv_image_dsc_t *flag_bitmap;
} panel_language_option_t;

static const panel_language_option_t s_panel_lang_opts[] = {
    {.code = "it", .label = "Italiano", .flag_bitmap = &g_flag_it_24x16},
    {.code = "en", .label = "English", .flag_bitmap = &g_flag_en_24x16},
    {.code = "de", .label = "Deutsch", .flag_bitmap = &g_flag_de_24x16},
    {.code = "fr", .label = "Français", .flag_bitmap = &g_flag_fr_24x16},
    {.code = "es", .label = "Español", .flag_bitmap = &g_flag_es_24x16},
};
...
void lvgl_page_language_show(void)
{
    ...
    for (size_t i = 0; i < (sizeof(s_panel_lang_opts) / sizeof(s_panel_lang_opts[0])); i++) {
        ...
        if (opt->flag_bitmap) {
            ...
            lv_image_set_src(img, opt->flag_bitmap);
