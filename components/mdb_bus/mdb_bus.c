...
static mdb_bus_status_t s_mdb_status = {0};
...
static void mdb_coin_sm(void) {
    ...
    switch (s_mdb_status.coin.state) {
        ...
        case MDB_STATE_INIT_RESET:
            ...
            mdb_bus_send_packet(MDB_ADDR_COIN_CHANGER | MDB_CMD_RESET, NULL, 0);
        ...
        case MDB_STATE_INIT_SETUP:
            ...
            mdb_bus_send_packet(MDB_ADDR_COIN_CHANGER | MDB_CMD_SETUP, NULL, 0);
            if (mdb_bus_receive_packet(rx, sizeof(rx), &rx_len, 50) == ESP_OK) {
        ...
        case MDB_STATE_IDLE_POLLING:
            mdb_bus_send_packet(MDB_ADDR_COIN_CHANGER | MDB_CMD_POLL, NULL, 0);
            ret = mdb_bus_receive_packet(rx, sizeof(rx), &rx_len, 20);
            if (ret == ESP_OK) {
