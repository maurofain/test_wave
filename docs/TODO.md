...
5. Operazioni su epaper : devono seguire il flusso operativo : attesa credito - scelta programma - esecuzione e autorepeat - saluti. 
   ...
   - si passa in modalià invertita quando siamo in preFineCiclo.  Il colore va riportato a normale non appenza inizia l'autoripetizione o termina il definitivamente il ciclo. 
   - Rimane operativo il cambio programma come nelle versioni con display e quindi va aggiornato il nome e tempo restante.
   - Alla fine di ogni programma mostriamo il messaggio 'Grazie!' per 3 secondi e ritorniamo in attesa credito/scelta programma se il credito è finito o in visualizzazione credito se c'è ancora credito disponibile.
...
8. Funzioni input i2c
   ...
   - pin IN03 : se OFF la scelta del device di uscita grafica ? fissato dagli IN01 e IN02 . Se ON il device va letto da config.json
...
16. Verifica la gestione generale dei componenti hardware
   - Tutti i component che gestiscono hardware devono avere queste funzioni e generare un report sullo stato attuale:
     ...
     3. `deactivate`
     4. `setup`
     5. `status` : indica se ? `disattivato`, `attivato`, `offline` (attivato ma non operativo) o `online` (attivato e operativo).
    - Report verifica componenti status/recovery:
       - `MDB`: esegue polling periodico e pu? andare `offline`, ma il recupero automatico ? debole. In caso di errori ripetuti di setup pu? autospegnersi logicamente e non sempre completa una reinizializzazione autonoma.
...
1.  la gettoniera non va mai disabilitata . se esiste un credito 
  - Va inibito MDB se si ? ricevuto VDC da QRCODE e riattivato quando viene sottratto l'ultimo credito QRCODE
  - Va inibito QRCODE se si ? ricevuto VDC dallo MDB e riattivato quando viene sottratto l'ultimo credito MDB
...
4.  al touch dello schermo in ADS riproduci buongiorno.wav se il credito ECS+VCS ? 0 oppure seleziona.wav se credito > 0, alla presentazione di MDR riproduci grazie.wav, all'avvio della ripetizione automatica del programma riproduci ripetizione.wav
...
47. ? Tasto ESCI in pagina programmi:
    ...
    - ? Visibilit? controllata da `allow_exit_programs_clears_vcd` e stato FSM (non visible quando RUNNING/PAUSED)
...
Il credito ? espresso in coin (1 coin = 1?). Le sorgenti sono:
...
5. Cliente pu? cambiare programma durante l'erogazione ? ricalcolo `rdu` e relay mask
...
ti descrivo cosa mi aspetto (una mia simulazione) cos? puoi confrontare il flusso attuale e correggerlo:
- rilevazione TAG
I (105808) MDB_CEVT: ****************************************
