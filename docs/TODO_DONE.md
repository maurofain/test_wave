...
- [x] Allineato il payload `payment`: `paymenttype` ora e' una stringa codificata secondo la specifica server (`CASH`, `CREC`, `BCOIN`, `COIN`, `SATI`, `VOUC`, `CASHL`, `WALL`)
- [x] Corretto il tipo di pagamento QR in `tasks_payment_type_from_session_source` da `SATI` a `WALL` per inviare `paymenttype: WALL`
