...
- Factory 
 ...
 1. Modifiche CCTalk
 2. 
 3. Fare valutazione per le funzioni di caricamento da remoto su chiamata degli artefatti immagini, tabelle testi e del firmware stesso. Considerare che questi contenuti possono essere salvarti sia in SPIFFS che in SD
 4. riorganizzazione tabelle localizzazione
    ...
    13. 
    14. modifica della Coda di eventi da utilizzare nell FSM e nei moduli di controllo: 
    15. partendo dalla definizione attuale di fsm_input_event_t definiamo che ogni agente (task o funzione di origine o destinazione di un messaggio) deve possedere un suo id unico chiamato agn_id di tipo uint8_t, quindi aggiungiamo dei campi alla struttura:
    ...
    16. ogni task analizza sotto mutex la lista dei messaggi per verificare se nei destinari ci sia il suo agn_id, se lo trova  lo toglie dalla lista , sblocca il mutex, e se somma di tutti i valori dell'array destinatari=0 elinima il messaggio dalla queue. Infine esegue quanto previsto dal messaggio 
    17. venno identificati tutti gli agenti, metterli in una tabella ed assegnare gli agn_id
    18. preparare una tabella con tutte le azioni ed assegnare gli action_id
    19. tutti i messaggi includono automaticamente l'agente per i log
 5. Crea un mini file manager per ispezionare il contenuto della SD e deiSPIFFS , con possibilità di caricare scaricare e cancellare file - solo sulla root senza folder. Si accede a questa funzione con un tasto nella Home. E' disponibile sia in App che in Factory.
 6. ✅ aggiungiamo altri tasti di pagamento all'emulatore : quelli attuali li definiamo come 'Crediti QR', mettiamo nella linea sotto una serie uguale con gli stessi valori come 'Crediti Tessera', poi sotto 2 tasti 'Monete' con valore 1 coin e 2 coins
 7. funzionamento macchina 
 ...
 8. Analisi criticità (riferita al punto 2 - coda eventi FSM):
 ...
 9. Piano test endpoint e funzioni (da riprendere)
 ...
 10. Chiamate server remoto: completare hardening/integrazione (gap analisi codice)
 ...
 11. lo scanner USB è gestito tramite la coda mesaggi?  
 12. la lettura di un codice QR deve passare a http_services che deve eseguire , se non già acquisito, il token tramite la chiamata login e quindi eseguire una chiamata a api_payment_post
 13. ✅ crea una funzione per visualizzare sullo schermo con il carattere a 48px la scritta 'Fuori servizio' ed eseguila nel caso di 'APP: [F] ERROR_LOCK attivo: avvio task inibito (reboot consecutivi=3)
'
...
## Eliminazione file `i18n_<lang>.map.json` dallo SPIFFS

### Contesto

I file `/spiffs/i18n_<lang>.map.json` (it, en, de, fr, es) contengono una mappatura ID numerico → nome simbolico per gli **scope** (9 entries) e le **key** (476 entries) del sistema di localizzazione i18n. Tutti e 5 i file sono **identici** tra loro (il map non è localizzato), e vengono letti a runtime per risolvere ID numerici nei loro nomi testuali durante la costruzione delle tabelle i18n.

**Spazio SPIFFS occupato**: 5 × 17.775 bytes = **86.8 KB** da liberare.

---

### Problema attuale

Il map viene caricato da SPIFFS con fopen/fread/cJSON_Parse ad ogni cambio lingua o richiesta tabella. Questo implica:
- I/O su SPIFFS a runtime (lento, soggetto a errori di montaggio)
- Allocazione heap per il cJSON parse tree (cache volatile)
- 5 file identici replicati per ogni lingua senza motivo
- Rischio di desync tra file su SPIFFS e costanti nel codice C

---

### Implementazione

#### 1. Creare `components/device_config/include/i18n_map_defs.h`

Header con due array statici in flash (`const char * const []`) e due funzioni di lookup inline:

```c
// Scope IDs (1-based, indice 0 non usato)
...
Gli array vengono dichiarati `static const char * const scopes[]` e `keys[]` marcati `__attribute__((section(".rodata")))` (automatico in ESP-IDF per `const` globali) → nessun consumo di RAM.

---

#### 2. Modifiche a `components/device_config/device_config.c`

**Rimuovere** (circa 80 righe):
- Variabili statiche `s_i18n_map_cache` e `s_i18n_map_lang`
- Funzione `_i18n_map_cache_clear()`
- Funzione `_build_i18n_map_path()`
- Funzione `_i18n_map_cache_build_for_lang()`
- Funzione `_lookup_map_text_from_id()`

**Sostituire in `_build_i18n_table_for_lang()`** (righe 308-345):
```c
// Prima (con map JSON):
...
// Dopo (con array statici):
const char *scope_text = i18n_scope_name(scope_id);
const char *key_text   = i18n_key_name(key_id);
```

**Sostituire in `device_config_get_ui_text_scoped()`** (righe 1738-1760):
```c
// Prima: scansione lineare su cJSON del map per trovare scope_id e key_id
if (_i18n_map_cache_build_for_lang(NULL) == ESP_OK && s_i18n_map_cache) {
    // ... doppio cJSON_ArrayForEach per risolvere nome → ID ...
}

// Dopo: lookup diretto sull'array in flash
...
Numero di call site toccate: **4** (2 lookup ID→nome + 2 lookup nome→ID).

---

#### 3. Modifiche a `components/web_ui/web_ui_common.c`

**Rimuovere** (circa 70 righe):
- Funzione `load_i18n_map_json()`
- Funzione `lookup_map_name()`

**Sostituire in `build_i18n_table_json()`** (righe 722-754):
```c
// Prima:
...
**Sostituire nel filtro scope per pagina** (riga 957):
```c
// Prima:
cJSON *map_root = load_i18n_map_json(language);
...
Numero di call site toccate: **3** (2 lookup in build_i18n_table_json + 1 nel filtro scope).

---

#### 4. Rimuovere i file dal SPIFFS

- Eliminare da `data/`: `i18n_it.map.json`, `i18n_en.map.json`, `i18n_de.map.json`, `i18n_fr.map.json`, `i18n_es.map.json`
- Aggiornare `CMakeLists.txt` o la definizione della partizione SPIFFS se i file erano elencati esplicitamente
...
| Cosa | Azione | File |
|---|---|---|
| 5 file `*.map.json` | **eliminare** da `data/` e SPIFFS | `data/` |
