...
static esp_err_t update_boot_reboot_guard(void)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "[F] Apertura NVS boot guard fallita: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    {
        ESP_LOGE(TAG, "[F] Lettura counter reboot fallita: %s", esp_err_to_name(ret));
        nvs_close(handle);
        return ret;
    }
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "[F] Salvataggio counter reboot fallito: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    ESP_LOGW(TAG, "[F] boot_guard: reset_reason=%d consecutive_reboots=%lu limit=%d",
             (int)reason, (unsigned long)s_consecutive_reboots, BOOT_GUARD_REBOOT_LIMIT);

    if (s_error_lock_active)
    {
        ESP_LOGE(TAG, "[F] ERROR_LOCK attivo: troppi reboot consecutivi");
    }
...
static esp_err_t update_crash_pending_record(void)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "[F] [C] apertura NVS crash record fallita: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret == ESP_OK)
    {
        ESP_LOGW(TAG, "[F] [C] crash pending registrato: reason=%d", (int)reason);
    }
    return ret;
}
...
static esp_err_t try_send_pending_crash_record(void)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "[F] [C] apertura NVS preboot send fallita: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    {
        ESP_LOGW(TAG, "[F] [C] preboot crash send saltato: server non configurato");
        return ESP_OK;
    }
    ...
    if (url[0] == '\0')
    {
        ESP_LOGW(TAG, "[F] [C] preboot crash send saltato: URL non valida");
        return ESP_OK;
    }
    ...
    if (!client)
    {
        ESP_LOGE(TAG, "[F] [C] preboot crash send: client init fallita");
        return ESP_FAIL;
    }
    ...
    if (http_ret == ESP_OK && status >= 200 && status < 300)
    {
        ...
        if (ret == ESP_OK)
        {
            ESP_LOGI(TAG, "[F] [C] preboot crash send OK (status=%d), pending cleared", status);
        }
        return ret;
    }

    ESP_LOGW(TAG, "[F] [C] preboot crash send fallito (err=%s status=%d), pending mantenuto",
...
esp_err_t init_mark_boot_completed(void)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "[F] [C] apertura NVS boot_completed fallita: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret == ESP_OK)
    {
        ...
        ESP_LOGI(TAG, "[F] [C] boot completato: contatore reboot consecutivi azzerato");
    }
